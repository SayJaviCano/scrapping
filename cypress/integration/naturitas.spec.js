...
describe("Srapping", () => {
  ...
  function guardarPrecio() {
    cy.get(".product-item-price .special-price").each((precio, index) => {
  ...
  function guardarPrecio1() {
    cy.get(".product-item-price .old-price").each((precio1, index, lista) => {
      datos[index + 24 * (paginaActual - 1)].precio1 = precio1
        .text()
        .replace("Msrp Price", "")
        .replace("\n", "")
        .replace("€", "")
  ...
  after("Guardar en BBDD", () => {
    ...
    let nuevasLineas = "Nombre; Precio; Precio1; \n "; // cabecera
